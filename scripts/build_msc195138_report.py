...
KEY = "MSC-195138"
template = (ROOT / ".cursor/skills/coverage-validator/report-template.html").read_text(
    encoding="utf-8"
)
...
replacements = {
    ...
    "{{REQ_MAPPED_DETAIL}}": "R1, R2 mapped Â· R3 uncovered",
    "{{QA_SCOPE_SUMMARY}}": "3 items",
    "{{OPEN_GAPS_SUMMARY}}": "1 High Â· 3 Med",
        ...
        [
            {
                ...
                "state": "MERGED",
                "title": "RepsPoller â€” publish unpublished reps_events in timestamp order",
    ...
    "{{DEV_COVERED_LIST}}": """<li><strong>R1</strong> â€” <code>poller_service.py</code> streams unpublished <code>reps_events</code> ordered by <code>header.timestamp</code>; <code>test_poller_service.py</code> (404 lines, unit)</li>
<li><strong>R2</strong> â€” <code>helper.py</code> <code>create_wings_status_message</code> preserves <code>correlationMetadata</code> and UTC <code>header.timestamp</code>; <code>test_helper.py</code> asserts request id namespace (unit)</li>
<li><strong>R3</strong> â€” indirect: ordered SNS publish reduces Pick-before-PickRequested errors; no unit test for PrepNotRequired-without-PickRequested</li>""",
    "{{QA_HANDOFF_LIST}}": """<li><strong>R1/R2</strong> â€” <span class="badge badge-e2e">E2E</span> Execute FF Race Condition.xlsx scenarios (TC1â€“TC11) in INT/SIT â€” verify Mascot event order and fulfill job status</li>
<li><strong>R3</strong> â€” <span class="badge badge-manual">Manual</span> Confirm mascot-api-transform handles PrepNotRequired when PickRequested absent (not in linked PRs; no dedicated test case)</li>
<li><strong>Regression</strong> â€” Compare <code>pegasusId-statusBefore.csv</code> vs <code>pegasusId-statusAfter.csv</code> attachments on Jira</li>""",
...
<tr><td>R3</td><td>Handle PrepNotRequired when PickRequested may be missing (temporary Mascot fix)</td><td><span class="badge badge-partial">Partial</span></td><td><span class="badge badge-partial">Partial</span></td><td><span class="badge badge-shared">Shared</span></td><td><span class="badge badge-manual">Manual</span></td><td>Not in PR #22/#75; no test plan case â€” QA manual required</td></tr>""",
    "{{CORRECTLY_IMPLEMENTED_LIST}}": """<li><strong>pegasus-reps#22</strong> â€” RepsPoller publishes unpublished <code>reps_events</code> in <code>header.timestamp</code> order with SNS MessageGroupId; 404-line unit test suite</li>
<li><strong>pegasus-texttransform#75</strong> â€” Status events retain <code>urn:wbd:distribute:request-id</code> in correlationMetadata for Mascot status accuracy</li>
<li><strong>Test plan:</strong> Jira attachment <code>FF Race Condition.xlsx</code> â€” Scenarios sheet, 11 E2E race-condition scenarios (TC1â€“TC11)</li>
<li>Jira evidence attachments document before/after fulfill job status for regression analysis</li>""",
    "{{GAPS_LIST}}": "",
    "{{ASSUMPTIONS_LIST}}": """<li>Root cause fix is event publish ordering via RepsPoller + accurate status payloads via texttransform â€” not mascot-api-transform code in this validation</li>
...
<li>Both PRs merged and deployed to SIT per Jira comments (Allyson Yao, May 2026)</li>""",
    "{{ACTIONS_LIST}}": """<li><strong>QA:</strong> Execute FF Race Condition.xlsx E2E scenarios (TC1â€“TC11) in INT/SIT; attach Mascot links to ticket</li>
...
if tp_pct is None:
    ...
    replacements["{{TESTPLAN_GAPS_LIST}}"] = (
        '<li class="medium">No attached test plan â€” acceptance criteria not mapped to formal cases</li>'
    )
    replacements["{{GAPS_LIST}}"] = (
        '<li class="medium"><strong>Medium:</strong> No QMetry test plan parsed â€” re-attach or download FF Race Condition.xlsx</li>'
    )
else:
    ...
    note = (
        "Jira attachment <code>FF Race Condition.xlsx</code> â€” Scenarios sheet Â· "
    ...
    gaps = [
        f'<li class="medium"><strong>{r}</strong> â€” no auto-mapped test case in FF Race Condition.xlsx</li>'
        for r in uncovered
    ]
...
<li class="medium"><strong>Medium:</strong> Test plan covers R1/R2 only (66.7%) â€” no scenario for R3 PrepNotRequired-without-PickRequested</li>"""
    if incomplete:
        impl_gaps += f'<li class="medium"><strong>Medium:</strong> {incomplete} test case(s) missing full Given/When/Then in step text</li>'
    impl_gaps += '<li class="medium"><strong>Medium:</strong> Mascot UI E2E not in dev test suite â€” prod log examples in Jira comments need QA replay</li>'
