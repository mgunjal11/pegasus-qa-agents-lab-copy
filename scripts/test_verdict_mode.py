...
def test_pragmatic_pass_with_gaps_on_tp_below_100():
    verdict, css, _ = _verdict(100.0, 77.8, "0 High · 6 Med", mode="pragmatic")
    assert verdict == "Pass with gaps"
    assert css == "pass-gaps"


def test_pragmatic_pass_when_perfect():
    verdict, css, _ = _verdict(100.0, 100.0, "None", mode="pragmatic")
    assert verdict == "Pass"
    assert css == "pass"


def test_strict_never_pass_with_med_gaps():
    verdict, _, _ = _verdict(100.0, 100.0, "0 High · 1 Med", mode="strict")
    assert verdict == "Pass with gaps"


def test_strict_pass_only_when_perfect():
    verdict, css, _ = _verdict(100.0, 100.0, "None", mode="strict")
    assert verdict == "Pass"
    assert css == "pass"
