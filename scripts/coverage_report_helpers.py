...
REPORT_AGENT_NAME = "msc-code-coverage-validator"
...
SUMMARY_METRIC_INFO: dict[str, str] = {
    ...
    "Test plan acceptance criteria coverage": (
        "Share of acceptance criteria that have at least one mapped test case "
        "in the attached QMetry test plan with Given/When/Then steps."
    ),
    "QA scope remaining": (
...
def inject_report_footer(html: str) -> str:
    ...
    if footer in html:
        return html
    if re.search(r"<footer>\s*Generated by msc-code-coverage-validator\s*</footer>", html):
        return re.sub(
            r"<footer>\s*Generated by msc-code-coverage-validator\s*</footer>",
            footer,
            html,
            count=1,
        )
