...
template = (ROOT / ".cursor/skills/msc-code-coverage-validator/report-template.html").read_text(encoding="utf-8")
...
replacements = {
    ...
    "{{OPEN_GAPS_SUMMARY}}": "2 High · 3 Med",
    "{{OPEN_GAPS_CLASS}}": "metric-warn",
    "{{OPEN_GAPS_DETAIL}}": "No linked PR; test plan incomplete GWT; Monitor E2E; code 9000",
    "{{CI_LINE_COVERAGE}}": "NA",
...
uncovered = cov.get("uncoveredRequirements") or ["R1", "R2", "R3"]
gaps = [
    f'<li class="medium"><strong>{r}</strong> — no auto-mapped test case (review Caption Monitoring sheet manually)</li>'
    for r in uncovered
]
...
replacements["{{VERDICT_RATIONALE}}"] = (
    ...
    f"({tc_n} scenarios, {gwt_n}/{tc_n} full Given/When/Then); no PR linked; Monitor E2E and STATUS_ERROR 9000 need QA."
)
...
if tp_pct == 0 or tp_pct == 0.0:
    gaps += '<li class="medium"><strong>Medium:</strong> Test plan scenarios not auto-mapped to Jira acceptance criteria — review Caption Monitoring sheet</li>'
elif incomplete_gwt:
