...
- **§5 traceability:** Jira `R*` + LADR `L*` rows; **FR** / **NFR** / **Process** badges on every ID cell via `classify_requirement_type()`; LADR badge on `L*` rows; **Evidence** compact (2 paths + 1 test) with **clickable +N more** (`<details>`) to show all paths/tests; mapping cache unchanged; **Dev tests** = Covered / Partial / Missing; **QA scope** shows **None** when dev-covered — tooltip copy unchanged
