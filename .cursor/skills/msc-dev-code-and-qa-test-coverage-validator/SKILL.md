...
- **Info-icon tooltips** (`i` badge) on:
  ...
  - **Jira input readiness** panel + each checklist item (AC, PR, test plan, Confluence) — **green ✓** (`ready-ok`) when input is present, **red ✗** (`ready-missing`) when required input is absent; `normalize_jira_readiness_icons()` upgrades legacy `ready-warn` / `!` on re-enhance
...
**Quick links:** `build_quick_links()` uses `confluence_requirements.collect_ladr_page_links()` — LADR/design pages only (not grooming or deployment remote links). Inferred LADR without a fetched page may still resolve a wiki URL when Jira text or analysis cache mentions LADR.
