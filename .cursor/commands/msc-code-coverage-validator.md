...
8. **Report UI** — before write, run HTML through `apply_report_ui_enhancements()` in `scripts/coverage_report_helpers.py` (info-icon tooltips; tooltip layout v5 — last two table columns anchor to th right edge, e.g. Dev tests / CI status; footer — Developed by Mayur Gunjal).
